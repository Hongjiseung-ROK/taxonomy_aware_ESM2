...
def visualize_superkingdom_embeddings(model_path, vocab_path, output_dir):
    print("Visualizing Random Superkingdom Embeddings...")
    ...
    print(f"Selected Superkingdoms: {selected_names}")

    # 3. Visualize Rank 0 (Superkingdom)
    rank_idx = 0
    ...
    plt.title("Superkingdom Embedding Space (Random 4)", fontsize=16)
    plt.xlabel("PC1")
    plt.ylabel("PC2")
    plt.grid(True, alpha=0.3)
    
    out_file = os.path.join(output_dir, "rank_0_superkingdom_random_4.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocab_path", type=str, default="data/vocab/superkingdom_vocab.json")
    parser.add_argument("--output_dir", type=str, default="outputs")
    args = parser.parse_args()
    
    visualize_superkingdom_embeddings(args.model_path, args.vocab_path, args.output_dir)
